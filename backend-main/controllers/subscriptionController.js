...
// Returns current plan info for the logged-in user, including live project count
const status = async (req, res) => {
    try {
        ...
        const projectsUsed = await Project.countDocuments({ owner: req.user._id });
        const projectLimit = limits.projects === Infinity ? null : limits.projects;

        // Reset AI counter if new month
        const now     = new Date();
        const resetAt = user.aiMessagesResetAt ? new Date(user.aiMessagesResetAt) : null;
        const needReset = !resetAt ||
            now.getMonth()    !== resetAt.getMonth() ||
            now.getFullYear() !== resetAt.getFullYear();

        let aiUsed = user.aiMessagesUsed || 0;
        if (needReset) {
            aiUsed = 0;
            // Don't save here — aiMessageGuard handles saving on next request
        }
        ...
        res.json({
            ...
            aiMessages: {
                used:    aiUsed,
                limit:   limits.aiMessages === Infinity ? null : limits.aiMessages,
